...
{
    if (lo_send(address_, "/subscribe", "sss", msg["nick"].c_str(), 
                host_.c_str(), port_.c_str()) == -1)
        printf("OSC error %d: %s\n", lo_address_errno(address_), 
                lo_address_errstr(address_));

    lo_send(address_, "/position", "sfff", msg["nick"].c_str(), 
            0.12345678f, 1.2123f, 9.43434f);
